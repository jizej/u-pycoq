...
Lemma lt_x_xmax : x < 2 ^ (p + w).
  ...
  omega. auto with a. apply Z.le_ge. auto with a. apply Z.le_ge. auto with a. Qed.
...
(* KEEP TOGETHER> *)
  ...
  ring. apply Z.le_ge. auto with a. apply Z.le_ge. auto with a. rewrite H in p_M. simpl in p_M.
...
(* KEEP TOGETHER> *)
  ...
  rewrite Zpow_2_w_is_2Zpow_2_wm1. apply Zplus_lt_le_compat. apply Z.lt_le_trans with M. apply Zmod_lt_z_m. omega.
  assumption. apply Zlt_le_weak. apply Z.lt_le_trans with (2 ^ p). apply Zmod_lt_z_m. auto with a. apply Zle_pow_le.
  ...
  apply Zplus_lt_le_compat. apply Z.lt_le_trans with M. apply Zmod_lt_z_m. omega. assumption. 
  apply Zlt_le_weak. apply Z.lt_le_trans with (2 ^ p). apply Zmod_lt_z_m. auto with a. apply Zle_pow_le. auto with a. 
...
Lemma r''_small : _Z r'' < 2 ^ p.
  ...
  omega. exact r'_small. omega. apply Z.le_lt_trans with (_Z r'). omega. apply lt_z__Z. fold M. assumption. rewrite ltw_true.
  apply Z.lt_le_trans with M. assumption. exact le_M_2pp. fold M. assumption. Qed.
...
(* KEEP TOGETHER> *)
  ...
  rewrite H1. left. ring. rewrite H1. right. ring. omega. apply Z.le_lt_trans with (_Z r'). omega. apply lt_z__Z. fold M.
...
(* KEEP TOGETHER> *)
  ...
  replace (x mod M + M - M) with (x mod M). apply modred. apply Zmod_le_0_z. omega. apply Z.lt_trans with M.
...
Lemma xhex : forall (M : Mset) (x : Z_ (2 ^ (Zlog_sup M + w))),
  ...
  apply Z.le_lt_trans with ((2 ^ (Zlog_sup M + w) - 1) / 2 ^ w). apply Zdiv_le. exact lt02w.
  ...
  apply Zlog_sup_correct1. apply Z.lt_le_trans with 1. omega. apply Mset_l. apply logsuplew.
  exact lt02w. omega. cut (0 < 2 ^ w). omega. exact lt02w. apply Z.le_ge. apply Zlog_sup_correct1. 
  apply Z.lt_le_trans with 1. omega. apply Mset_l. apply Z.le_ge. exact le_0_w. Qed.
...
Lemma M'ex : forall M : Mset, in_Z_ (2 ^ w) (2 ^ (Zlog_sup M + w) / M - 2 ^ w).
  ...
  apply Z.le_trans with (M * 2 ^ w / M). rewrite Zmult_comm. rewrite Z_div_mult. omega. omega. 
  apply Zdiv_le. omega. apply Zmult_le_compat_r. elim (Zlog_sup_correct2 M). omega. omega. 
  apply Zlt_le_weak. exact lt02w. omega. apply Z.le_ge. exact le_0_w. 
  cut (2 ^ (Zlog_sup M + w) / M < 2 ^ w + 2 ^ w). omega. rewrite Zpower_exp.
  apply Z.le_lt_trans with (((2 * M - 1) * 2 ^ w) / M). apply Zdiv_le. omega. apply Zmult_le_compat_r.
  replace (Zlog_sup M) with (Zlog_sup M - 1 + 1). rewrite Zpower_exp. 
  apply Z.le_trans with ((M - 1) * 2). apply Zmult_le_compat. elim (Zlog_sup_correct2 M). omega.
  omega. simpl. unfold Zpower_pos. simpl. omega. apply Z.le_trans with (2 ^ (1 - 1)). simpl. omega.
  ...
  rewrite Zdiv_times_plus. cut (- 2 ^ w / M < 0). omega. apply Z.le_lt_trans with (-1 / M).
  apply Zdiv_le. omega. cut (0 < 2 ^ w). omega. exact lt02w. replace (-1) with (0 * M - 1).
  rewrite Zdiv_mult_minus. omega. omega. omega. omega. ring. omega. ring. omega. apply Z.le_ge. exact le_0_w.
...
Lemma p1ex_2le : forall M : Mset, in_Z_ w (Zlog_sup M).
  ...
  intro M. unfold in_Z_. split. apply Z.le_trans with (Zlog_sup 1). simpl. omega. 
  apply Zlog_sup_seq. apply Mset_l. apply Z.le_lt_trans with (w - 1).
...
Lemma tex : forall M : Mset, in_Z_ w (w - Zlog_sup M - 1 / M).
  ...
  apply Z.le_trans with (2 ^ (w - 1)). apply Mset_r. apply Zlt_le_weak. apply lt_2wm1_2w.
...
(* KEEP TOGETHER> *)
  ...
  apply Z.lt_gt. exact lt02w. exact lt02w. elim (M'ex M). omega. elim (M'ex M). omega. 
  apply Z.le_lt_trans with ((2 ^ (Zlog_sup M + w) - 1) / 2 ^ w). apply Zdiv_le. exact lt02w.
  cut (x < 2 ^ (Zlog_sup M + w)). omega. apply lt_z__Z. rewrite Zpower_exp. rewrite Zdiv_mult_minus.
  omega. exact lt02w. omega. cut (0 < 2 ^ w). omega. exact lt02w. apply Z.le_ge. apply Zlog_sup_correct1.
  apply Z.lt_le_trans with 1. omega. apply Mset_l. apply Z.le_ge. exact le_0_w. Qed.

Lemma xheex : forall (M : Mset) (h : Z_ (2 ^ Zlog_sup M)),
  in_Z_ (2 ^ w) h.

  intros M h. unfold in_Z_. split. apply le_0__Z. apply Z.lt_le_trans with (2 ^ (Zlog_sup M)).
  apply lt_z__Z. apply Zle_pow_le. apply Zlog_sup_correct1. apply Z.lt_le_trans with 1. omega. apply Mset_l.
  apply Z.le_trans with (Zlog_sup (2 ^ w)).
  apply Zlog_sup_seq. apply Zlt_le_weak. apply Z.le_lt_trans with (2 ^ (w - 1)). apply Mset_r.
...
(* KEEP TOGETHER> *)
  ...
  rewrite Zmod_minus_distr_r. rewrite Z_mod_same. apply f_equal2 with (f := Zmod). ring. trivial.
  apply Z.lt_gt. exact lt02w. exact lt02w. elim (M'ex M). omega. elim (M'ex M). omega. apply lt_z__Z. Qed.
